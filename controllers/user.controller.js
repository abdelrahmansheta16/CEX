...
exports.getETHBalance = async (req, res) => {
    try {
        ...
        const baseURL = `https://eth-sepolia.g.alchemy.com/v2/${process.env.API_KEY}`;
        ...
        const response = await axios(rpcData);
        const balanceHex = response.data.result;
        const balanceWei = parseInt(balanceHex, 16);
        return res.status(200).send({
            success: true,
            msg: "Get account balance",
            data: balanceWei
        });
    } catch (e) {
        return res.status(200).send({
            success: false,
            msg: 'Unable to get account ETH balance',
        });
    }
}